...
import ast.billmanagment.mybills.Utils.CustomAlertConfirmationInterface;
import ast.billmanagment.mybills.Utils.CustomAlertDialog;
import ast.billmanagment.mybills.Utils.IWebCallback;
import ast.billmanagment.mybills.Utils.IWebIndexedCallback;
import ast.billmanagment.mybills.Utils.IWebPaginationCallback;
import ast.billmanagment.mybills.Utils.RModel_Error;
import ast.billmanagment.mybills.Utils.RModel_Message;
import ast.billmanagment.mybills.Utils.RModel_onFailureError;


public class AppConfig {
    private static ast.billmanagment.mybills.AppConfig ourInstance;// = new AppConfig(null);
    ...
    public static void initInstance(Context _mContext) {
        if (ourInstance == null) {
            // Create the instance
            ourInstance = new ast.billmanagment.mybills.AppConfig(_mContext);
        }
    }

    public static ast.billmanagment.mybills.AppConfig getInstance() {
        return ourInstance;
    }
...
//                new BigDecimal(_distance).setScale(2, RoundingMode.HALF_UP).doubleValue()));
    ...
    public void navtoLogin() {
        ...
        if (mUser.isLoggedIn()) {
            ...
            try {
                Intent intent = new Intent(ast.billmanagment.mybills.MyApplication.mContext,
                        IntroActivity.class);

                // set the new task and clear flags
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                ast.billmanagment.mybills.MyApplication.mContext.startActivity(intent);
